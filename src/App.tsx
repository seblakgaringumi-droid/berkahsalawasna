              ...
              prev.map((item) => {
                if (item.product.id === updated.id) {
                  ...
                  return {
                    ...
                    product: updated,
                    quantity: newQty,
                    subtotal: newQty * updated.selling_price,
                  };
                }
                return item;
        ...
        return prev.map((item) =>
          item.product.id === product.id
            ? {
                ...item,
                quantity: nextQty,
                subtotal: nextQty * product.selling_price,
              }
            : item
        );
      } else {
        const initQty = Math.min(stock, quantity);
        return [
          ...prev,
          {
            product,
            ...
            subtotal: initQty * product.selling_price,
          },
        ];
      }
    });
    ...
    setCartItems((prev) =>
      prev.map((item) => {
        if (item.product.id === productId) {
          ...
          const cappedQty = Math.min(stock, newQty);
          return {
            ...item,
            quantity: cappedQty,
            subtotal: cappedQty * item.product.selling_price,
          };
        }
        return item;
      })
